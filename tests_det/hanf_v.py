...
overrides_1 = {"task": "pose",
               "mode": "predict",
               "model": r'../trains_pose/hands_08_08_m/train4/weights/best.pt',
               "verbose": False,
               "iou": 0.25,
               "save": False,
               "conf": 0.5,
...
video_path = r"F:/hand_pose_2.mp4"
...
while cap.isOpened():
    ...
    for i, bbox in enumerate(hand_all.boxes.xyxy):
        ...
        if len(shou_all.boxes.conf) > 0:
            ...
            res = kp_predictor.predict(result)
            l_0 = res[0] if res[1] > 0.5 else "no_gesture"
            resnet_label = f"kptnt: {l_0} conf: {round(res[1], 2)}"
            cv2.putText(frame, resnet_label, (x11, y11 - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        ...
        resnet_res = resnet_pred(image_shou)
        l_1 = resnet_res[0] if resnet_res[1] > 0.5 else "no_gesture"
        resnet_label = f"resnt: {l_1} conf: {round(resnet_res[1], 2)}"
        cv2.putText(frame, resnet_label, (x11, y11 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        shufflenet_res = shufflenet_pred(image_shou)
        l_2 = shufflenet_res[0] if shufflenet_res[1] > 0.5 else "no_gesture"
        shufflenet_label = f"shfnt: {l_2} conf: {round(shufflenet_res[1], 2)}"
        cv2.putText(frame, shufflenet_label, (x11, y11 - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
